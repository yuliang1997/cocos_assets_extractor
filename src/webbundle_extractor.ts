...
export function extract(input: string) {
  ...
  const configPath = findFile(input, "config", "json", false)
  console.log(`configPath:${configPath}`)
  assert(configPath)
  const configContent = fs.readFileSync(configPath, { encoding: "utf-8" })
  const configJson = JSON.parse(configContent) as IConfigOption
  processOptions(configJson)
  const outBase = `${input}/../out_${path.basename(input)}`
  // writeJson(outBase, "config.out", configJson)
  for (const packId in configJson.packs) {
    const packIds = configJson.packs[packId]
    const packPath = findFile(input + "/import", packId, "json", true)
    console.log(`packPath:${packPath}`)
    assert(packPath)
    const packContent = fs.readFileSync(packPath, { encoding: "utf-8" })
    const packJson = JSON.parse(packContent)
    ...
    for (let packIdx = 0; packIdx < packIds.length; packIdx++) {
      ...
      switch (type) {
        case "sp.SkeletonData": {
          ...
          // write skel
          const srcSkelPath = findFile(`${input}/native/`, uuid, ext, true)
        ...
        case "cc.SpriteFrame": {
          ...
          const atlasPath = findFile(`${input}/native/`, pureUuid, "*", true)
